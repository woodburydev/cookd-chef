...
import { Animated, Dimensions, Easing, StyleSheet, Touchable, TouchableNativeFeedback, View } from 'react-native';
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.contentContainerStyles}>
          <View style={styles.TextContainerStyle}>
            <Text style={[commonStyles.mt20, styles.welcomeBackText]} type="description">Welcome Back Chef!</Text>
            <Text style={commonStyles.mx20} type="large-header">Your Upcoming Jobs</Text>
          </View>

          <ScrollView contentContainerStyle={styles.cardsContainer} horizontal={true} showsHorizontalScrollIndicator={false}>
            {
              jobData.map((item, index) => (
                <TouchableWithoutFeedback style={[styles.cardWrapper]}>
                  <View style={[commonStyles.mx10, styles.topLeftCard]}>
                    <View>
                      <Text type="label" style={commonStyles.mb5}>{item.date}</Text>
                      <Text type="description">{item.timeRange}</Text>
                      <Text style={styles.cardGuestCount} type="description">{item.guests} Guests</Text>
                      <Text style={styles.cardGuestCount} type="description">${item.cost}</Text>
                    </View>
                    <View />
                    <View>
                      <Image
                  ...
                  <View style={[commonStyles.mx10, styles.midCard]}>
                    <Text type="label" style={commonStyles.mb5}>Client {index + 1}</Text>
                    <Text>{item.address}</Text>
                  </View>
                  <View style={[commonStyles.mx10, styles.lowerCard]}>
                    <Text type="label" style={commonStyles.mb5}>Order: {item.order.menu}</Text>
                    {
                    ...
                    }
                    {
                      item.specialInstructions && (
                        <Text style={[commonStyles.mx10, styles.cardGuestCount, styles.welcomeBackText]}>*special instructions*</Text>
                      )
                    }
                  </View>
          ...
          <View style={styles.TextContainerStyle}>
            <Text style={commonStyles.mt30} type="large-header">Your Schedule</Text>
            <Text style={[{ fontWeight: '600', top: 20 }, styles.welcomeBackText]}>Jobs This Week</Text>
            <BarChart
              segments={3}
              width={width - 40}
              height={300}
